...
class PTSampler(object):
    ...
    def PTMCMCOneStep(self, p0, lnlike0, lnprob0, iter):
        ...
        if self.resume and self.resumeLength > 0 and iter < self.resumeLength:
            ...
            param_ind = None
            while True:
                y, qxy, jump_name, jump_ind = self._jump(p0, iter, jump_ind=jump_ind, param_ind=param_ind)
                # compute prior and likelihood
                lp = self.logp(y)
                if lp == -np.inf:
                    # get the parameter index, to draw the same parameter again
                    param_ind = np.argwhere((p0 - y) != 0).squeeze()
                    print("Calling {} again because of -inf prior".format(jump_name))
                    continue
                ...
                if ll == -np.inf:
                    # get the parameter index, to draw the same parameter again
                    param_ind = np.argwhere((p0 - y) != 0).squeeze()
                    print("Calling {} again because of -inf likelihood".format(jump_name))
                else:
                    break

            self.jumpDict[jump_name][0] += 1

            newlnprob = 1 / self.temp * ll + lp
            ...
            if diff > np.log(np.random.rand()):
                ...
                # accept jump
                p0, lnlike0, lnprob0 = y, ll, newlnprob
    ...
    def _jump(self, x, iter, jump_ind=None, param_ind=None):
        """
        Call Jump proposals
        ...
        # call function
        ind = np.random.randint(0, length) if jump_ind is None else jump_ind
        try:
            q, qxy = self.propCycle[ind](x, iter, 1 / self.temp, param_ind=param_ind)
        except TypeError:
            # function does not support parameter index input
        ...
        return q, qxy, self.propCycle[ind].__name__, ind
