...
class Ultranest(_TemporaryFileSamplerMixin, NestedSampler):
    ...
    def run_sampler(self):
        ...
        region_class = self.kwargs.pop("region_class", None)
        ...
        ndims = self.search_parameter_keys
        print("ndims is: {}".format(ndims))
        if stepsampler is not None:
            #if isinstance(stepsampler, ultranest.stepsampler.StepSampler):
            if stepsampler == "step_sampler":
                sampler.stepsampler = ultranest.stepsampler.SliceSampler(nsteps=2*len(ndims),generate_direction=ultranest.stepsampler.generate_mixture_random_direction, adaptive_nsteps="move-distance")
            else:
                logger.warning(
        ...
        if region_class is not None:
            #if isinstance(region_class, ultranest.mlfriends.RobustEllipsoidRegion):
            if region_class == "ellipsoid":
                sampler.region_class = ultranest.mlfriends.RobustEllipsoidRegion
            elif region_class == "simple":
                sampler.region_class = ultranest.mlfriends.SimpleRegion
            else:
                logger.warning(
                    "The supplied region class is not the correct type. "
                    "The default region class will be used instead."
                )
