...
class HyperModel(object):
    ...
    def setup_sampler(self, outdir='chains', resume=False, sample_nmodel=True,
        ...
        if 'monopole_log10_A' in self.snames:
        ...
        # BWM prior draw
        if 'bwm_log10_A' in self.snames:
            print('Adding BWM prior draws...\n')
            sampler.addProposalToCycle(jp.draw_from_bwm_prior, 10)

        # FDM prior draw
        if 'fdm_log10_A' in self.snames:
            print('Adding FDM prior draws...\n')
            sampler.addProposalToCycle(jp.draw_from_fdm_prior, 10)

        # CW prior draw
        if 'cw_log10_h' in self.snames:
            print('Adding CW prior draws...\n')
            sampler.addProposalToCycle(jp.draw_from_cw_log_uniform_distribution, 10)

        if 'cw_log10_Mc' in self.snames or 'cw_circ_log10_Mc' in self.snames or 'cw_ecc_log10_Mc' in self.snames:
            print('Adding CW prior draws...\n')
            sampler.addProposalToCycle(jp.draw_from_cw_prior, 10)


        # # Prior distribution draw for CW parameters
        # if any([str(p).split(':')[0] for p in list(self.params) if 'cw' in str(p)]):
        #     print('Adding cw param prior draws...\n')
